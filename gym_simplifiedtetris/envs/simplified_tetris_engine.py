...
class SimplifiedTetrisEngine(object):
    ...
    def __init__(
        ...
        self._sleep_time = 1  # Determines the speed at which the game is played
        # self._sleep_time = 0  # Determines the speed at which the game is played
    ...
    def _get_param_grid(self):
        """
        Return the grid provided in the parameter
        :return: array
        """
        return self._grid
    ...
    def _resize_grid(self, grid: np.ndarray) -> None:
        ...
        self._img = self._img.resize(
            (self._width * self.CELL_SIZE, self._height * self.CELL_SIZE), 0
        )
    ...
    def _get_dellacherie_scores(self) -> np.array:
        ...
        print(f"Prios: {max_indices}")
        # print(f"Prios: {self._get_priorities(max_indices)}")
    ...
    def _get_holes(self) -> int:
        ...
        :return: holes.
        """
        # print(np.count_nonzero((self._grid).cumsum(axis=1) * ~self._grid))
    ...
    """
    Anthony's Changes to the Existing Engine:
    """

    def _get_last_piece_info(self):
        """
        Return the array generated regarding the state of the last piece placed on the game board.
        :return: a set of info (rows_added_to, landing_height, eliminated_num_blocks, num_rows_cleared)
        """
        return self._last_move_info
