...
class SimplifiedTetrisBaseEnv(gym.Env):
    ...
    """
    Anthony's Changes to the Existing Tetris Environment:
    """
    @property
    def get_grid(self):
        return self._engine._get_grid()

    @property
    def _get_last_piece_info(self):
        return self._engine._get_last_piece_info()

    @property
    def _get_param_grid(self):
        return self._engine._get_param_grid()

    @property
    def _get_score(self):
        return self._engine._score

    @property
    def _get_mean_score(self):
        return self._engine._get_mean_score()
    ...
    def tempStep(self, action: int) -> Tuple[np.array, float, bool, Dict[str, Any]]:
        """
        Hard drop the current piece according to the action. Terminate the
        game if the piece cannot fit into the bottom 'height-piece_size' rows.
        Otherwise, select a new piece and reset the anchor.
        ...
        # Keep information to undo
        self._oldEngine = deepcopy(self._engine)
        # self._oldEngine = [self._engine._piece._rotation,
        #                    self._engine._anchor,
        #                    self._engine._final_scores,
        #                    self._engine._score,
        #                    self._engine._piece]
    ...
    def undoStep(self):
        self._engine = self._oldEngine

        # self._engine._piece._rotation = self._oldEngine[0]
        # self._engine._anchor = self._oldEngine[1]
        # self._engine._final_scores = self._oldEngine[2]
        # self._engine._score = self._oldEngine[3]
        # self._engine._piece = self._oldEngine[4]
