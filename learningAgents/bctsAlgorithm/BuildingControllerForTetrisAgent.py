...
class BuildingControllerForTetrisAgent(object):
    """
    ...
    # The BCTS algorithm relies on 8 features (the first 6 are from Dellacherie's Algorithm):
    # 1. Landing Height
    ...
    def calcHoleDepth(self):
        ...
        for i in range(self.boardWidth):
            for j in range(self.boardHeight):
                if (j != self.boardHeight - 1) and (self.grid[i][j] == True) and (self.grid[i][j + 1] == False):
                    # Find the first hole. From the first hole, all other filled cells will be considered "depth" for
                    # this hole. Once we have calculated for the whole column, break (move to next column).
                    subseq = self.grid[i][j : self.boardHeight - 1]
                    sum += np.count_nonzero(subseq == True)
        return sum

    def calcRowsHoles(self):
        """
        Returns the number of rows that have at least one hole.
        :return: an integer
        """
        results = [0] * self.boardHeight # 20 rows
        for i in range(self.boardWidth):
            for j in range(self.boardHeight):
                if (j != self.boardHeight - 1) and (self.grid[i][j] == True) and (self.grid[i][j + 1] == False):
                    results[j] = 1
        ...
        #
        # rotatedToRows = np.rot90(self.grid)
        # print(rotatedToRows)
        # # print(self.grid)
        #
        # sum = 0
        ...
        #         if (j != self.boardWidth - 1) and (rotatedToRows[j][i] == True) and (rotatedToRows[j][i + 1] == False):
        #             sum += 1
        #             # If we find a hole in a row, we move to the next row
        #             break
        # return sum

    def getBCTSForState(self):
        """
        ...
        return np.array([self.calcLandingHeight(), self.calcErodedPieces(), self.calcRowTransitions(),
                         ...
                         self.calcHoleDepth(), self.calcRowsHoles()],
                        dtype="double"
                        )

    def calcBCTSAlgo(self, env, weights):
        ...
        for action in range(self.numActions):
            ...
            feature_values = self.getBCTSForState()
    ...
    def predict(self, env, weights):
        ...
        """
        return np.argmax(self.calcBCTSAlgo(env, weights))
